...
// ── handlePhotoUploadPreview: commit imgData to form state and show preview ──
function handlePhotoUploadPreview(imgData, fileInfo) {
	// Persist into hidden field consumed by sendpdf()
	document.getElementById('photo_data').value = imgData;

	// Hide the drop-zone label; show preview area
	var dropZone = document.getElementById('os_photo_msg');
	if (dropZone) dropZone.style.display = 'none';

	var prevImg = document.getElementById('photo-preview-img');
	if (prevImg) prevImg.src = imgData;

	var prevArea = document.getElementById('photo-preview-area');
	if (prevArea) prevArea.style.display = 'block';

	var infoEl = document.getElementById('photo-file-info');
	if (infoEl && fileInfo) infoEl.textContent = fileInfo;

	// Hide any prior error
	var errEl = document.getElementById('photo-error-msg');
	if (errEl) errEl.style.display = 'none';

	// Show validation checklist
	var checklist = document.getElementById('photo-checklist');
	if (checklist) checklist.style.display = 'block';

	unhide();
}

// ── removeUploadedPhoto: reset upload panel so user can retake ───────────────
function removeUploadedPhoto() {
	document.getElementById('photo_data').value = '';
	canvas_status('photo', 0);

	// Reset file input
	var inp = document.getElementById('os_photo');
	if (inp) inp.value = '';
...
function os_camera_upload() {
	...
	if (file.size > maxBytes) {
		var errEl = document.getElementById('photo-error-msg');
		if (errEl) { errEl.textContent = 'Image file is too large (max 10 MB). Please choose a smaller file.'; errEl.style.display = 'block'; }
		return false;
	}

	// Build file info label  e.g.  "photo.jpg · 2.4 MB  3024×4032"
	var fileSizeMB = (file.size / (1024 * 1024)).toFixed(1);
	var fileInfoBase = file.name + ' \u00b7 ' + fileSizeMB + ' MB';

	hide('Resizing image...');

	// resize image using JS (FileReader path — all modern browsers)
	if (window.File && window.FileReader) {
		...
		reader.onload = function (evt) { 
			...
			// apple is buggy. 90deg is perfect. -90 is upside down.
			img.src = img_data;
	
			img.onload = function() {
				// Resolution check: warn if image is smaller than 1000px wide
				var natW = img.naturalWidth || img.width;
				var natH = img.naturalHeight || img.height;
				var fileInfo = fileInfoBase + '  ' + natW + '\u00d7' + natH;

				if (natW < 1000 && natH < 1000) {
					var errEl = document.getElementById('photo-error-msg');
					if (errEl) { errEl.textContent = 'Image is too small (' + natW + '\u00d7' + natH + 'px). Please use a higher-resolution photo.'; errEl.style.display = 'block'; }
					unhide();
					return;
				}
				...
				handlePhotoUploadPreview(img_data, fileInfo);
			}
		}
		reader.readAsDataURL(file);
		return true;
	} else {
		// Fallback: server-side resize (old Safari / WebView)
		var fd = new FormData();
