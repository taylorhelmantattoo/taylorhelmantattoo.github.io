...
// ── Two-step flow ─────────────────────────────────────────────────────────────
// Only active when $settings['two_step'] is true (set in configs/tattoo_new.php)
if (!empty($settings['two_step']) && !isset($_GET['print']) && !isset($_GET['topdf'])) {

    // ── Step A: artist POST — generate token and show link ────────────────────
...
<div class="two-step-wrap">
  ...
  <a class="back-link" href="?step=artist&release=<?=htmlspecialchars($_GET['release'] ?? 'tattoo')?>">← Generate another link</a>
</div>
<script>
...
<div class="two-step-wrap">
  <div class="two-step-card">
    ...
    <form method="post" action="?step=artist&release=<?=htmlspecialchars($_GET['release'] ?? 'tattoo')?>">
      ...
      <span class="clear-btn" onclick="clearCanvas('signature_artist')">Clear</span>
      <div class="sig-wrap">
      ...
      <br /><br />
      <button type="submit" name="artist_step_submit" class="btn">Generate Client Link →</button>
    </form>
  </div>
</div>
<script>
// canvas_status is normally in ajax.js which isn't loaded on this page
...
document.addEventListener('DOMContentLoaded', function() {
    // Size the canvas to its actual rendered pixel dimensions before init
    // so mouse coordinates map correctly in Chrome
    var c = document.getElementById('signature_artist');
    var rect = c.parentElement.getBoundingClientRect();
    c.width  = Math.floor(rect.width)  || 472;
    c.height = 150;
    // Now run the standard signature init (clearCanvas, event listeners)
    ...
    document.querySelector('form').addEventListener('submit', function(e) {
        ...
        if (!status || status.value !== 'yes') {
            alert('Please draw your signature before generating the client link.');
            e.preventDefault();
            return false;
        }
...
<?php
    ...
    // ── Step B: client opens token link ───────────────────────────────────────
...
?>
		...
		<div class="value">
			<?php if(!$submit){?>
			<input type="text" name="signature_client_text" id="signature_client_text" />
			<?php } else { ?>
...
<?php if($settings['enable_camera'] && $skin != "print") {?>
	...
	?>
		...
		<div class="value">
			<?php if(!$submit){?>
			<input type="text" name="signature_photo_text" id="signature_photo_text" />
			<?php } else { ?>
			<div class="fake_input"><?=htmlspecialchars($_POST['signature_photo_text'] ?? '')?></div>
			<?php } ?>
		</div>
		<div class="break"></div>
	</fieldset><?php
